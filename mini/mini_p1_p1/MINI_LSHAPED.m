...
sizemesh = input('Inserire la dimensione della mesh (Valore compreso tra 0 e 10) = ');
f(1,1) = input('Inserire forzante di superficie direzione x = '); 
f(2,1) = input('Inserire forzante di superficie direzione y = ');
g(1,1) = input('Inserire forzante sul lato sinistro direzione x = ');
g(2,1) = input('Inserire forzante sul lato sinistro direzione y = ');
young  = input('Inserire modulo di Young = ');
%-------------------------------------------------------------------------%
pv =[10.0, 0.0; 20.0, 0.0; 20.0, 20.0; 0.0, 20.0;...
    0.0, 10.0; 10.0, 10.0; 10.0, 0.0];
[coordinates,element] = ...
    distmesh2d(@dpoly,@huniform,sizemesh,[0,0; 20,20],pv,pv);
...
%-------------------------------------------------------------------------%
[UD,UF] = BDlshaped(coordinates,edge2element,exterioredge);
[bg] = BNlshaped(coordinates,UF,g,ngdlu);
%
for jj = 1:size(UD,1)
     I = 2*UD(jj,1);
     II= 2*UD(jj,1)-1;
