function [UD,UF] = BDcooks(coordinates,edge2element,exterioredge)
...
for i = 1:size(exterioredge,1)
     ...
     if (coordinates(edge2element(exterioredge(i,1),2),1) == 0)
         UD(J,1) = edge2element(exterioredge(i,1),2);
         J = J+1;
     end
     
     if (coordinates( edge2element(exterioredge(i,1),1),1) == 48 && ...
         coordinates( edge2element(exterioredge(i,1),2),1) == 48)
